...
def auth():
    ...
    response = requests.post(
        current_app.config["TAKE_ACCESS_TOKEN_ENDPOINT"],
        data={
    ...
    try:
        ...
        error_description = json.loads(
            html.unescape(response.get("error_description", "{}"))
        )
        return (
            jsonify(
                {
                    ...
                    "messages": {
                        "_schema": [
                            error_description.get("message", "Service Error")
                        ]
                    },
                }
            ),
    ...
    try:
        user = (
            db.session.query(
                UserSession.user_session_id, UserSession.user_token
            )
            .filter(
        ...
        try:
            with db.session.begin_nested():
                user = UserSession(
                    sha512(str(time()).encode() + urandom(128)).hexdigest(),
                    access_token,
                    refresh_token,
                    expires_in,
                    now,
            ...
            created = True
    if not created:
        db.session.query(UserSession).filter(
            UserSession.user_session_id == user.user_session_id
